#!/usr/bin/env python3
"""Manage version numbers across the oecluster project.
...
def _locations() -> list[VersionLocation]:
    ...
    return [
        # C++ header defines
        VersionLocation(
            file=PROJECT_ROOT / "include" / "oecluster" / "oecluster.h",
            label="C++ header (MAJOR)",
            pattern=r"#define OECLUSTER_VERSION_MAJOR \d+",
            replacement="#define OECLUSTER_VERSION_MAJOR {major}",
            extract=r"#define OECLUSTER_VERSION_MAJOR (\d+)",
        ),
        VersionLocation(
            file=PROJECT_ROOT / "include" / "oecluster" / "oecluster.h",
            label="C++ header (MINOR)",
            pattern=r"#define OECLUSTER_VERSION_MINOR \d+",
            replacement="#define OECLUSTER_VERSION_MINOR {minor}",
            extract=r"#define OECLUSTER_VERSION_MINOR (\d+)",
        ),
        VersionLocation(
            file=PROJECT_ROOT / "include" / "oecluster" / "oecluster.h",
            label="C++ header (PATCH)",
            pattern=r"#define OECLUSTER_VERSION_PATCH \d+",
            replacement="#define OECLUSTER_VERSION_PATCH {patch}",
            extract=r"#define OECLUSTER_VERSION_PATCH (\d+)",
        ),
        # CMakeLists.txt
        VersionLocation(
            file=PROJECT_ROOT / "CMakeLists.txt",
            label="CMakeLists.txt",
            pattern=r"(project\(oecluster VERSION )\d+\.\d+\.\d+",
            replacement=r"\g<1>{major}.{minor}.{patch}",
            extract=r"project\(oecluster VERSION (\d+\.\d+\.\d+)",
        ),
...
@click.group()
def cli():
    """Manage oecluster version numbers across all project files."""
...
def get():
    ...
    table = Table(
        title="oecluster version numbers",
        box=box.ROUNDED,
        show_lines=True,
